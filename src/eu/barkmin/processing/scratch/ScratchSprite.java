...
public class ScratchSprite {
	...
	public boolean isTouchingEdge() {
		...
		float[] cornerBottomLeft = ScratchStage.rotateXY(x - spriteWidth / 2.0f, y + spriteHeight / 2.0f, x, y,
				rotation);
		float[] cornerBottomRight = ScratchStage.rotateXY(x + spriteWidth / 2.0f, y + spriteHeight / 2.0f, x, y,
				rotation);
	...
	/**
	 * @return true, if, if this sprite was drawn, it would overwrite a pixel, which
	 *         currently has the value of colour, and false otherwise
	 */
	public boolean isTouchingColour(int colour) {
		ScratchStage.parent.loadPixels();
		int[] pixelsApp = ScratchStage.parent.pixels;
		int[] pixelsSelf = this.getPixels();
		int startPixelPosition = ((int) this.getX()) - this.getPixelWidth() / 2
				+ ((int) this.getY() - this.getPixelHeight() / 2) * ScratchStage.parent.pixelWidth;

		for (int p = 0; p < pixelsSelf.length; p++) {
			if (ScratchStage.parent.alpha(pixelsSelf[p]) != 0) {
				int pixelWidth = this.getPixelWidth();
				int pixelPosition = startPixelPosition + ScratchStage.parent.pixelWidth * (p / pixelWidth)
						+ p % pixelWidth;
				if (pixelPosition >= 0 && pixelPosition < pixelsApp.length) {
					if (pixelsApp[pixelPosition] == colour) {
						return true;
					}
				}
			}
